...
_vue.default.config.devtools = false; // toast.test.js里的代码和plugin.js是完全分离的，没有任何关系
...
describe('Toast', function () {
  ...
  describe('props', function () {
    it('toast 接受 autoClose　属性', function (done) {
      ...
      }).$mount(div); // 确保在vm.execAutoClose执行后再去判断vm.$el是否被移出document.body
    ...
    it('toast 接受 closeButton', function () {
      ...
      var vm = new Constructor({
        propsData: {
          closeButton: {
            text: '测试text',
            callback: callback // sionon的callback,方便expect

          }
        }
      }).$mount();
      var closeButton = vm.$el.querySelector('.close');
      expect(closeButton.textContent.trim()).to.equal('测试text'); //触发 closeButton 的 click 事件,相当于closeButton.click()

      var event = new Event('click');
      closeButton.dispatchEvent(event);
      expect(callback).to.have.been.called;
    });
    it('toast 接受 enableHTML', function () {
      var Constructor = _vue.default.extend(_toast.default);
      ...
      vm.$mount(); // 能通过文本选择器选到标签,就说明测试成功

      expect(vm.$el.hasChildNodes('strong')).to.equal(true);
    });
    it('toast 接受 position', function () {
      var Constructor = _vue.default.extend(_toast.default);
      ...
      }).$mount(); // 测试classList包含position-middle即可
