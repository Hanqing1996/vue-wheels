...
_vue.default.config.devtools = false; // Vue.component('g-button', Button)
...
describe('Button', function () {
  it('存在.', function () {
    expect(_button.default).to.exist; // 不是假值，就不报错
  });
  ...
  describe('事件', function () {
    ...
    it('点击 button 触发 touch 事件', function () {
      vm = new Constructor({
        propsData: {
          icon: 'settings'
        }
      }).$mount();
      var callback = sinon.fake(); // 为组件实例设置监听事件

      vm.$on('click1', callback); //触发 vm.$el的button 的 click 事件,等价于buttonElement.click()

      var event = new Event('click');
      var buttonElement = vm.$el;
