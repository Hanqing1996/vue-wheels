...
_vue.default.component('g-col', _col.default); // new Vue 和组件注册没有任何关系
...
try {
  // 测试按钮含有 icon
  {
    var Constructor = _vue.default.extend(_button.default); // 生成一个 vue 实例
    ...
    expect(href).to.eq('#i-settings'); // 卸载vue实例
  ...
  {
    var _Constructor = _vue.default.extend(_button.default); // 生成一个 vue 实例
...

  {
    ...
    var _Constructor2 = _vue.default.extend(_button.default); // 生成一个 vue 实例
    ...
    var svg = _vm2.$el.querySelector('svg'); // 获取 button 的子元素 svg


    var _window$getComputedSt = window.getComputedStyle(svg),
        order = _window$getComputedSt.order; // 获取 svg 样式（一个对象）
...

  {
    ...
    var spy = _chai.default.spy(function () {}); // 监听当前实例上的自定义事件 touch，回调函数为 spy


    _vm4.$on('touch', spy); // 注意 vm 是一个vue实例，button 是一个真实的选择器（DOM节点）


    var button = _vm4.$el; // 触发 button 的 click 事件

    button.click(); // 期待 button 的 click 事件被触发后，spy 被调用
