...
describe('Input', function () {
  ...
  describe('props', function () {
    ...
    it('接收 error', function () {
      ...
      expect(useElement.getAttribute('xlink:href')).to.equal('#i-error'); // 检测 errorMessage
  ...
  describe('事件', function () {
    ...
    it('支持 change/input/focus/blur 事件', function () {
      ['change', 'input', 'focus', 'blur'].forEach(function (eventName) {
        ...
        var callback = sinon.fake(); // 为vue实例添加监听事件，注意这与index.html中有无为input添加事件无关

        vm.$on(eventName, callback); //触发input的change 事件，index.html 中的 input 没有添加 change,focus,blur 事件
